...
class CommentService:
    ...
    async def get_comments(self,post_id:str,sort_by):
        ...
        data=await self.CommentAction.get_all_comments_with_user(post_id)
        return data
...
class CommentsActionService:
    ...
    async def report(self,comment_id:str,reported_by:str,data: dict):
        ...
        self.CommentAction.validate_object_id(comment_id)
        comment=await self.CommentAction.get_data_by_id(comment_id,{"created_by":1})
        if comment["created_by"]==reported_by:
